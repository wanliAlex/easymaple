...
def make_capture(fixture: str = 'test_frame.jpg') -> Capture:
...
# ---------------------------------------------------------------------------
# Minimap bounds detection — requires test_frame.jpg with visible minimap
# ---------------------------------------------------------------------------
...
class TestFindMinimapBounds:
    def test_finds_bounds_in_real_frame(self, test_frame):
        ...
        if bounds is None:
            pytest.skip(
                "Minimap not detected in tests/fixtures/test_frame.jpg — "
...
class TestDetectPlayer:
    ...
    def test_relative_coords_in_valid_range_when_player_found(self, test_frame):
        ...
        bounds = cap._find_minimap_bounds(test_frame)
        if bounds is None:
            pytest.skip("minimap not detected in test_frame.jpg")
        mm_tl, mm_br = bounds
        minimap = test_frame[mm_tl[1]:mm_br[1], mm_tl[0]:mm_br[0]]
        pos = cap._detect_player(minimap)
        if pos is None:
            pytest.skip("player icon not visible in test_frame.jpg minimap")
        x, y = pos
...
class TestFileFrameSource:
    def test_grab_returns_numpy_array(self):
        src = FileFrameSource(FIXTURES / 'test_frame.jpg')
