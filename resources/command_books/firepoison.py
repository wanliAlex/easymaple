...
class Flame_Sweep(Command):
    ...
    def main(self):
        ...
        for _ in range(self.repetitions):
            press(Key.FLAME_SWEEP, self.attacks, up_time=0.05)
        key_up(self.direction)
        if self.attacks > 2:
            time.sleep(0.3)
        else:
...
class Flame_Haze(Command):
    """Uses 'Tengu Strike' once."""

    def main(self):
        press(Key.FLAME_HAZE, 1, up_time=0.05)
...
class DoT_Punisher(Command):

    def main(self):
        press(Key.DOT_PUNISHER, 3)


class Poinson_Nova(Command):

    def main(self):
        press(Key.POISON_NOVA, 2)


class Meteor_Shower(Command):

    def main(self):
        press(Key.METEOR_SHOWER, 2)
    ...
    def main(self):
        if self.direction:
            ...
            if config.player_pos[0] > 0.5:
                ...
                press('right', 1, down_time=0.1, up_time=0.05)
        press(Key.CREEPING_TOXIN, 2)

class Poison_Chain(Command):
    """Uses 'Nine-Tailed Fury' once."""

    def main(self):
        press(Key.POISON_CHAIN, 3)
